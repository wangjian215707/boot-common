    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o) throws Exception {
        ...
        if (null != securityContextImpl) {
            ...
            if (!StringUtils.isEmpty(password) && !StringUtils.isEmpty(userName)) {
                ...
                ICECookieUtil.addCookie(httpServletResponse, domain, token.getTokenValue());
                ((CredentialsContainer) securityContextImpl.getAuthentication()).eraseCredentials();//清除密码
            }
            if (StringUtils.isEmpty(password)) {//pc端访问
                String cookie = ICECookieUtil.getDecodedCookieValue(httpServletRequest, "iPlanetDirectoryPro");
