...
//    private UserService service;
    ...
    @Autowired
    private IUserWxUserService service;
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if(null!=user){
            String tgt= tgtService.getTicketGrantingTicket(username, DigestUtil.md5(password));
            log.info("tgt:"+tgt);
            if(!StringUtils.isEmpty(tgt)){
                TSysWxUser wxUser=service.findById(openid);
                log.info("---------取到用户");
                if(null == wxUser){
