...
public class DBUtil {
    ...
    @Transactional(readOnly = true)
    public <T> List<T> getAllBeanList(SqlEnum sql, Class<T> beanClass) {
        List<T> results = jdbcTemplate.query(sql.sql(dataType), BeanPropertyRowMapper.newInstance(beanClass));
        return results;
    }

    @Transactional(readOnly = true)
    public <T> List<T> getAllBeanList(SqlEnum sql, Class<T> beanClass, Object... params) {
        params = deleteNull(params);
        List<T> results = jdbcTemplate.query(sql.sql(dataType), BeanPropertyRowMapper.newInstance(beanClass), params);
        return results;
    }

    @Transactional
    public <T> T queryFieldValue(SqlEnum sql, Class<T> beanClass) {
        List<T> results = jdbcTemplate.query(sql.sql(dataType), BeanPropertyRowMapper.newInstance(beanClass));
        if (null != results && results.size() > 0) {
            return results.get(0);
        }
     ...
     */
    @Transactional(readOnly = true)
    public <T> T queryFieldValue(SqlEnum sql, Class<T> beanClass, Object... params) {
        params = deleteNull(params);
        List<T> results = jdbcTemplate.query(sql.sql(dataType), BeanPropertyRowMapper.newInstance(beanClass), params);
        if (null != results && results.size() > 0) {
            return results.get(0);
